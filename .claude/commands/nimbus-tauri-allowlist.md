...
Every JSON-RPC method the renderer is permitted to call via `rpc_call`. Currently 60 entries. The list is:

- **Alphabetized** — enforced by `allowlist_is_alphabetized`. Insert in order; do not append at the end.
- **Size-asserted** — `allowlist_exact_size` checks `ALLOWED_METHODS.len() == 60`. **Adding a method requires updating this constant.** Removing a method also requires updating it.
