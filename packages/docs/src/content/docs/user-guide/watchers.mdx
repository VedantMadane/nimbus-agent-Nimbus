...
connector sync completes. When the condition is satisfied, you receive a
notification in the system tray and an audit-log entry is written. Optionally,
a watcher can trigger a saved workflow automatically.
...
3. A row is inserted into `watcher_event` containing the condition snapshot
