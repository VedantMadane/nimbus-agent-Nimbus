...
```
Every gate must be ✓. The fast tier is **33 gates** today — do not hard-code
that number here: `scripts/lib/preflight-gates.ts` is the manifest, a drift
test fails when a CI gate is missing from it, and this line was stale at "18"
for long enough that the `tail` window no longer showed the whole run. If `bun.lock` was touched (or any dep change), also: `bun install --frozen-lockfile` must exit 0 (else CI's "Setup Nimbus CI" fails for every job).
