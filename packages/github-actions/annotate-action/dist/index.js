...
async function postAnnotation(gatewayUrl, token, payload, timeoutMs) {
  ...
  try {
    ...
    if (res.status === 429) {
      ...
      return retryAfter === null ? { status: "rate_limited" } : { status: "rate_limited", retryAfter };
    }
    if (res.status === 503) {
      let hint;
      try {
        const body2 = await res.json();
        if (typeof body2.hint === "string")
          hint = body2.hint;
      } catch {}
      return hint === undefined ? { status: "surface_disabled" } : { status: "surface_disabled", hint };
    }
