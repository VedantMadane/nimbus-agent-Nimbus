...
The GitHub Actions connector is a deeper read surface on top of the same `GITHUB_PAT` used by the [GitHub connector](/connectors/github/), focused on workflows, runs, jobs, and logs. It also exposes two HITL-gated write tools for triggering and cancelling runs.
...
The GitHub Actions connector reuses the same Vault key as the [GitHub connector](/connectors/github/) — `github.pat` — and is injected at spawn as `GITHUB_PAT`. Authenticate once; both connectors work.
...
- [GitHub connector](/connectors/github/) — shares the same PAT
