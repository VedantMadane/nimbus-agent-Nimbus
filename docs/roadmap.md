...
- [ ] **T6 PR 1 — I10 timing-safe helper consolidation** — migrate `ipc/lan-pairing.ts`'s local `timingSafeEqual` and `ipc/http-auth.ts`'s local `constantTimeStringEqual` into one canonical helper under `util/`; finishes the I10 consolidation that `extensions/verify-extensions.ts` and `updater/updater.ts` already follow. Updates `SECURITY-INVARIANTS.md` I10 row and `security-invariants.test.ts`. No migration.
- [ ] **T6 PR 2 — `tool_call_log` audit table (V29)** — closes "Structured tool-call result auditing (S8-F10)" below; complements the `<tool_output>` envelope (I11) by recording the envelope's contents at audit time. New `audit.toolCalls` IPC method (read-only, LAN-callable, not in Tauri allowlist). Wired at both `wrapToolOutput` sites in `engine/agent.ts` and `connectors/lazy-mesh/mesh.ts`.
